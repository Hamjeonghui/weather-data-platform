...
@Component
public class KmaVilageFcstClient {

    private static final Logger log = LoggerFactory.getLogger(KmaVilageFcstClient.class);
    ...
    public KmaVilageFcstClient(RestClient.Builder restClientBuilder,
                                @Value("${kma.service-key}") String serviceKey) {
        this.restClient = restClientBuilder.build();
        this.serviceKey = serviceKey;
    }

    public List<Map<String, Object>> fetchVilageFcst(String baseUrl, long nx, long ny, String baseDate, String baseTime) {
        List<Map<String, Object>> allItems = new ArrayList<>();
        int pageNo = 1;

        while (true) {
            KmaResponse response = call(baseUrl, nx, ny, baseDate, baseTime, pageNo);
        ...
        if (allItems.isEmpty()) {
            log.warn("단기/초단기예보 API 응답에 자료 없음: nx={}, ny={}, baseDate={}, baseTime={}", nx, ny, baseDate, baseTime);
    ...
    private KmaResponse call(String baseUrl, long nx, long ny, String baseDate, String baseTime, int pageNo) {
        ...
        try {
            ...
            log.warn("단기/초단기예보 API 호출 실패: nx={}, ny={}, baseDate={}, baseTime={}", nx, ny, baseDate, baseTime, e);
            throw new BusinessException(ErrorCode.EXTERNAL_API_ERROR);
        }
    }
    ...
    private void validate(KmaResponse response, long nx, long ny, String baseDate, String baseTime) {
        if (response == null || response.response() == null || response.response().header() == null
            ...
            log.warn("단기/초단기예보 API 응답 실패: nx={}, ny={}, baseDate={}, baseTime={}, response={}",
